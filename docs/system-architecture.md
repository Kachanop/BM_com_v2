...
| **Backend Worker (สำรอง)** | โค้ด Cloudflare Worker ที่เตรียมไว้สำหรับงานที่ควรทำผ่านเซิร์ฟเวอร์ (เช่น เรียกผู้ให้บริการภายนอกสำหรับตรวจสลิปโอนเงินอัตโนมัติในอนาคต) ปัจจุบันหน้าเว็บจริงยังไม่เรียกใช้ ส่งกลับแค่ mock data |
| **Hosting** | แผนวางไฟล์ frontend ที่ build แล้วบน Cloudflare Pages ให้บุคคลทั่วไปเข้าถึงได้ |

## เหตุผลเชิงสถาปัตยกรรมที่สำคัญ

1. **ไม่มีเซิร์ฟเวอร์กลางสำหรับงานพื้นฐาน (CRUD)** — เพราะ Supabase มี RLS ในตัว ลดความซับซ้อนและเวลาพัฒนาของโปรเจกต์ขนาดเล็ก-กลางแบบนี้ (รายละเอียดการตัดสินใจอยู่ใน [analysis-and-design.md](./analysis-and-design.md))
2. **แยก Backend Worker ไว้ต่างหากล่วงหน้า** — เผื่อในอนาคตต้องเรียกบริการภายนอก (เช่น ตรวจสลิปธนาคาร) ซึ่งไม่ควรทำจากฝั่งเบราว์เซอร์โดยตรง (เสี่ยงเรื่อง API key รั่วไหล) จึงเตรียมโครงไว้แต่ยังไม่ผูกกับ flow จริง
